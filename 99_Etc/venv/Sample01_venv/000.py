...
def Wavelet0(u, j, k):
	result = []
	for _u in u:
		_v = ( 2 ** ( j / 2) ) * Epanechnikov( (2 ** j) * _u - k)
		result.append(_v)
	return result
...
#    ax.plot(u, ft_Pinsker, color='m', linewidth=3.0, label="Pinsker Kernel (1.56) [p27]")
    ...
    wavelet25 = Wavelet0(u, 0, 0)
    ax.plot(u, wavelet25, color='b', linewidth=1.0, label="Wavelet base (1.89)")

    wavelet45 = Wavelet0(u, 1, 0)
    ax.plot(u, wavelet45, color='g', linewidth=1.0, label="Wavelet base (1.89)")

    wavelet65 = Wavelet0(u, 2, 0)
    ax.plot(u, wavelet65, color='r', linewidth=1.0, label="Wavelet base (1.89)")


#    ft = ft_Epane + ft_Silverman + ft_Spline + ft_Pinsker

    wavelet = wavelet25 + wavelet45 + wavelet65

    y_min = math.floor(min(wavelet)*10) / 10 - y_tick
    y_max = math.ceil(max(wavelet)*10) / 10 + y_tick
